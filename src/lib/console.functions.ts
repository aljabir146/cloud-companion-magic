...
function mockExec(host: string, port: number, username: string, command: string) {
...
export const consoleExec = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context as any;
    const { host, port, username, password, command, nodeId, sessionId } = data;

    let result: { ok: boolean; output: string; exit_code?: number } = { ok: true, output: "" };

    // Try real exec via node-agent if a remote node is selected
    if (nodeId) {
      const { data: node } = await supabaseAdmin
        .from("nodes")
        .select("api_url, agent_secret, kind, verify_ssl")
        .eq("id", nodeId)
        .maybeSingle();
      if (node?.api_url && node.agent_secret && node.kind === "remote") {
        try {
          const body = JSON.stringify({ host, port, username, password, command });
          ...
          result = parsed && typeof parsed.output === "string"
            ? { ok: !!parsed.ok, output: parsed.output, exit_code: parsed.exit_code }
            : { ok: res.ok, output: text };
        } catch (err: any) {
          result = { ok: false, output: `agent error: ${err?.message || "unreachable"}` };
        }
      } else {
        result = mockExec(host, port, username, command);
      }
    } else {
      result = mockExec(host, port, username, command);
    }

    // Persist to session log (skip clear screen sentinel)
